...
$query = "SELECT * FROM data_kader WHERE id_kader = '$id'";
...
while($row = mysqli_fetch_assoc($data)){
     ...
     $nik = $row['nik_kader'];
    $nama = $row['nama_kader'];
    $tempat_lahir = $row['tmp_lahirkd'];
    $tanggal_lahir = $row['tgl_lahirkd'];
    $jenis = $row['jk_kader'];
    $agama  = $row['agama_kader'];
    $sk = $row['sk_nokd'];
    $pendidikan = $row['pendidikan_kader'];
    $pekerjaan = $row['pekerjaan_kader'];
    $jabatan = $row['jabatan_kader'];
    $goldar = $row['goldar_kader'];
    $alamat = $row['alamat_kader'];
      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content"><br />
            <form method="post" action="pro_editkader.php" id="demo-form2" data-parsley-validate enctype="multipart/form-data" class="form-horizontal form-label-left">
              <!-- GANTI JADI NAMA POSYANDU DARI TABEL DATA POSYANDU-->
              <div class="form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="tingakat">Bagian Kader<span class="required">*</span></label>
                  ...
                  <select class="select2_group form-control" name="bagian">
                    ...
                    <option value="pokja"<?php if($bagian=="pokja"){echo "selected=\"selected\"";} ?>>Pokja Desa</option>
                    <option value="posyandu"<?php if($bagian=="posyandu"){echo "selected=\"selected\"";}?>>Posyandu</option>   
                    <option value="bkb"<?php if($bagian=="bkb"){echo "selected=\"selected\"";}?>>BKB</option>     
                    <option value="bkl"<?php if($bagian=="bkl"){echo "selected=\"selected\"";}?>>BKL</option>
                    <option value="bkr"<?php if($bagian=="bkr"){echo "selected=\"selected\"";}?>>BKR</option>   
                    <option value="up2k"<?php if($bagian=="up2k"){echo "selected=\"selected\"";}?>>UP2K</option>  
                    <option value="uppks"<?php if($bagian=="uppks"){echo "selected=\"selected\"";}?>>UPPKS</option>  
                  </select>
                  </div>
              </div>  
              <div class="form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nik"> NIK<span class="required">*</span></label>
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    ...
                    <input type="text" id="id" name="id" required="required" value="<?php echo $id; ?>" class="form-control col-md-7 col-xs-12">             
                  </div>
              </div>
             <div class="form-group">
             ...
             <div class="form-group">
              ...
              <div class="form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="alamat"> Jabatan <span class="required">*</span></label>
                   <div class="col-md-6 col-sm-6 col-xs-12">
                    <input type="jabatan" id="jabatan" name="jabatan" required="required" value="<?php echo $jabatan; ?>" class="form-control col-md-7 col-xs-12">             
                  </div>
              </div>  
              <div class="form-group">
